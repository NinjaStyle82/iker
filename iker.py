...
def launchProccess (command):
    ...
    if len(error) > 0 and "ERROR" in error[0].decode("UTF-8") and "port 500" in error[0].decode("UTF-8"):
...
def updateProgressBar (top, current, transform):
    ...
    progressbar = "[....................] %d%% - Current transform: %s"
    tt = 20
    step = top // tt
    # Progress: [====================] 10% : DES-MD5
    cc = current // step
    ...
    print(progressbar % (perctg, transform), end='\r', flush=True)
...
def checkIkeScan ():
    ...
    if "ike-scan" in output.lower().decode("UTF-8"):
        return True
    else:
        return False
                ...
                for hsh in HASHLIST:
                    for auth in AUTHLIST:
                        for group in GROUPLIST:
                            ...
                            process = launchProccess ("%s -M --aggressive -P%s_handshake.txt --trans=%s,%s,%s,%s --id=test %s" % (FULLIKESCANPATH, ip, enc, hsh, auth, group, ip))
                            process.wait()
...
def enumerateGroupID (args, vpns):
    ...
    for ip in list(vpns.keys()):
        ...
        process.wait()
        message1 = sub (r'(HDR=\()[^\)]*(\))', r'\1xxxxxxxxxxx\2', process.stdout.readlines()[1].decode("UTF-8").strip() )
        
        delay (DELAY)
        
        process = launchProccess ("%s --aggressive --trans=%s --id=badgroupiker654321 %s" % (FULLIKESCANPATH, vpns[ip]["aggressive"][0][0], ip))
        process.wait()
        message2 = sub (r'(HDR=\()[^\)]*(\))', r'\1xxxxxxxxxxx\2', process.stdout.readlines()[1].decode("UTF-8").strip() )
        
        delay (DELAY)
        
        process = launchProccess ("%s --aggressive --trans=%s --id=badgroupiker935831 %s" % (FULLIKESCANPATH, vpns[ip]["aggressive"][0][0], ip))
        process.wait()
        message3 = sub (r'(HDR=\()[^\)]*(\))', r'\1xxxxxxxxxxx\2', process.stdout.readlines()[1].decode("UTF-8").strip() )
        ...
        try:
            ...
            for cid in fdict:
                ...
                process = launchProccess ("%s --aggressive --trans=%s --id=%s %s" % (FULLIKESCANPATH, vpns[ip]["aggressive"][0][0], cid, ip))
                process.wait()
                msg = sub (r'(HDR=\()[^\)]*(\))', r'\1xxxxxxxxxxx\2', process.stdout.readlines()[1].decode("UTF-8").strip() )
